...
- **Makefile** commands are instructions executed by the **make** utility to build or manage projects. 

- These commands are defined within a **Makefile** and are associated with specific targets.
...
- **targets**: are specific make rules to be called during the makefile exeuction
- **Dependencies**_ specify the required dependencies (e.g., other targets and files) needed to execute the target.
- **command**: refers to any shell command to be executed
...
- **Icarus Verilog** is a Verilog HDL **compiler** and **simulator**. 

- Using **Icarus** primarily involves two main commands:    
  - **iverilog** for compilation 
  - **vvp** for simulation.


## iverilog (Compiler Command)
...
### Common iVerilog Flags {.alert}
iVerilog accepts several flags
...
# Example iVerilog Workflow

### Compile your design {.example}
...
### Visualize the Waveforms (Optional)
...
For more information about the usage of iverilog you can check this link [https://steveicarus.github.io/iverilog/](https://steveicarus.github.io/iverilog/) 

# Makefile example for icarus
...
- This makefile target automatically performs the following acctions. 
  ...
  - It delets de `sim_build` directory and all its content, and creates a new empty `sim_build`directory. This directory is used for keeping all simulation and generated files organized. - Compiles the verilog files using icarus
...
In this case, when running in the terminal **`make all`** it automatically will execute all individual simulations. 
...
**%** is a makefile internal wildcard which will serve as a placeholder in a for-each manner. In this case, we can specify any text after **`show_`** and **`make`** will try to match that text in the prerequisites. On the other hand, (**$^**) means all prerequisites of the target. 
...
## Makefile example for Verilator
...
More information regarding veriltaror and its command line arguments please refer to [https://veripool.org/guide/latest/exe_verilator.html](https://veripool.org/guide/latest/exe_verilator.html) 
...
- This makefile target automatically performs the following acctions. 
	...
	- Compiles the verilog files using Verilator commands as just explained previuosly.
...
- This make command will automatically compile and execute the simulation. Also, the simulation ouput `VCD` will be produced when enabled.


## Full Makefile example for RGB Mixer
...
- In this way, you can automatically execute a simulation of multiple or single designs by simply typing the make command. 
- Notice that it is not necesary to manually execute the whole command, instead you just execute **`make`** and the desired target. 
...
A Makefile contains all recipies used for automating any flow that involves compilation, simulation etc. You can simply call any of the commands from the shell w/o the need of call the whole command every time you need it.  
